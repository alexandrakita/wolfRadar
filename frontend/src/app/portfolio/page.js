...
export default function Page() {
  ...
  const requestRemove = (sym) => setHoldingToRemove(sym);

  const confirmRemoveHolding = () => {
    if (!holdingToRemove) return;
    remove(holdingToRemove);
    toast.success(TOAST.holdingRemoved(holdingToRemove));
  ...
  return (
    <Fragment>
      <DashboardPageShell>
        <PageHeading
          title="Portfolio"
          description="Overview of your holdings, returns, and performance over time."
          className="mb-8"
          actions={
            <Button
              onClick={openAdd}
        ...
        {isEmpty ? (
          <PortfolioEmptyState onAddHolding={openAdd} />
        ) : (
          <Fragment>
                    ...
                    >
                      <defs>
                        <linearGradient id="pfGrad" x1="0" y1="0" x2="0" y2="1">
                          ...
                          <stop
                            offset="100%"
                            stopColor="oklch(0.62 0.22 270)"
                            stopOpacity={0}
                          />
                        </linearGradient>
                      </defs>
                      <CartesianGrid
                        stroke="oklch(0.3 0.025 262)"
                        strokeDasharray="3 3"
                        vertical={false}
                      />
                      <XAxis
                      ...
                      <Tooltip
                        ...
                        formatter={(value) => [fmtMoney(value), "Value"]}
                      />
                      <Area
                        type="monotone"
                        dataKey="v"
              ...
              <PanelSurface>
                <div className="px-5 py-4">
                  ...
                  <p className="text-sm text-muted-foreground">
                    All positions and their performance
                  </p>
                </div>
      ...
      <ConfirmDeleteDialog
        ...
        onOpenChange={(open) => {
          if (!open) setHoldingToRemove(null);
        }}
        title={holdingToRemove ? `Remove ${holdingToRemove}?` : "Remove holding?"}
        description={
          holdingToRemove
            ? `${holdingToRemove} will be removed from your portfolio. This cannot be undone.`
