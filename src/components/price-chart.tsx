...
import { getYahooChart, type ChartRange } from "@/lib/yahoo.functions";
...
export function PriceChart({ symbol }: { symbol: string }) {
  ...
  const fetchChart = useServerFn(getYahooChart);

  const { data, isLoading, isFetching } = useQuery({
    queryKey: ["chart", symbol, range],
    queryFn: () => fetchChart({ data: { symbol, range } }),
    staleTime: range === "1D" ? 30_000 : 5 * 60_000,
  });

  const points = data?.points ?? [];
  const { first, last, up } = useMemo(() => {
    if (!points.length) return { first: 0, last: 0, up: true };
    const f = points[0].c;
    const l = points[points.length - 1].c;
    return { first: f, last: l, up: l >= f };
  }, [points]);
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {points.length ? `${points.length} data points · ${range}` : "Historical price"}
          </p>
        </div>
        <div className="flex items-center gap-4">
          {points.length ? (
            <div className={cn("flex items-center gap-1 text-sm font-medium tabular-nums", up ? "text-accent" : "text-destructive")}>
